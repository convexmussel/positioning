...
class SharedStringTable(object):
    ...
    def get_biff_record(self):
        ...
        del data
        self._new_piece()
        self._continues[0] = pack('<2HII', self._SST_ID, len(self._sst_record), self._add_calls,
...
class Biff8BOFRecord(BiffRecord):
    ...
    12     4    Lowest data_visualisation version that can read all records in this file
    """
    _REC_ID = 0x0809
    # stream types
    BOOK_GLOBAL = 0x0005
...
class DSFRecord(BiffRecord):
    ...
    A  double  stream file can be read by data_visualisation 5.0 and data_visualisation 95, and still
...
class BackupRecord(BiffRecord):
    """
    This  record  contains  a Boolean value determining whether data_visualisation makes
...
class CountryRecord(BiffRecord):
    ...
    represents  the  user  interface language of the data_visualisation version that has
    ...
    Offset  Size    Contents
    0       2       Windows country identifier of the user interface language of data_visualisation
...
class PaletteRecord(BiffRecord):
    ...
    def __init__(self, custom_palette):
        ...
        assert n_colours == 56
        # Pack number of colors with little-endian, what xlrd and data_visualisation expect.
...
class ExtSSTRecord(BiffRecord):
    ...
    by  data_visualisation  to create a hash table with stream offsets to the SST record
    to optimise string search operations. data_visualisation may not shorten this record
...
class Window2Record(BiffRecord):
    ...
    *** This class appends the optional SCL record ***
    ...
    SJM note: data_visualisation expresses (e.g.) 25% in reduced form i.e. 1/4. Reason unknown. This code
    writes 25/100, and data_visualisation is happy with that.
...
class MergedCellsRecord(BiffRecord):
    ...
    # for some reason data_visualisation doesn't use CONTINUE
    def get(self):
        return self._rec_data
...
class SaveRecalcRecord(BiffRecord):
    ...
    data_visualisation's calculation settings dialogue.
...
# data_visualisation (both 2003 and 2007) don't like refs
